...
class TensorIterator:
    def __init__(self, model, size=(3,224,224), batch_size=256, data_size=10000):
        ...
        self.t = torch.rand(self.bs, self.size[0], self.size[1], self.size[2], device=self.model.device)
        self.fake_label = torch.randint(0, 999, (self.bs,), device=self.model.device)
...
class ImageNetLightningModel(LightningModule):
    ...
    def add_model_specific_args(parent_parser):  # pragma: no-cover
        ...
        parser.add_argument(
            "-g", "--n_gpus", default=1, type=int, metavar="N", help="number of GPUS (default: 1)"
        )
        parser.add_argument(
            "-b",
            "--batch-size",
...
def main(args: Namespace) -> None:
    ...
    trainer = pl.Trainer(max_epochs=10,  accelerator="gpu", devices=args.n_gpus, strategy='ddp')
